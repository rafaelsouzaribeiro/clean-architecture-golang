...
func (suite *EventDispatcherTestSuite) TestEventDispatcherRegisterSameHandle() {
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
...
func (suite *EventDispatcherTestSuite) TestEventDispatcherClean() {
	// Event 1
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
...
func (suite *EventDispatcherTestSuite) TestEventDispatch_Dispatch() {
	eh := &MockHandler{}
	eh.On("Handle", &suite.event).Once()

	eh2 := &MockHandler{}
	eh2.On("Handle", &suite.event2).Twice()
